...
let afiliados = [
  ...
  },
  {
    ...
    descricao:
      "Sem segredos! Curso sobre renderização completamente em Revit Construção do loft Aplicação de texturas Dicas de iluminação artificial Iluminação natural Como criar render 360º grátis pela autodesk Como criar QR Code Gerar PDF Projeto de interiores",
...
$(document).ready(function() {
  ...
  for (i = 0; i < 4; i++) {
    $("#afiliados ul").append(
      `<li class="afiliado"><h2>${afiliados[i].title}</h2><p>${
        afiliados[i].descricao
      }</p>  <h3>Preview</h3><iframe    src="${
        afiliados[i].link
      }"    frameborder="0"    sandbox="allow-popups"  ></iframe></li>`
    );
  }
